...
async function safeApiCall<T>(apiCall: () => Promise<T>, errorMessage: string): Promise<T> {
  try {
    ...
    if (error instanceof Error) {
        // Refina mensagens de erro comuns para serem mais amigáveis em português
        if (error.message.includes('API key not valid')) {
            throw new Error('Sua chave de API não é válida. Verifique a configuração na Netlify e tente novamente.');
        }
        if (error.message.includes('quota')) {
             throw new Error('Você excedeu sua cota de uso da API. Verifique seu plano no Google AI Studio.');
        }
        if (error.message.includes('timed out')) {
            throw new Error('A requisição demorou muito para responder. Verifique sua conexão e tente novamente.');
        }
        // Para outros erros da API, podemos relançá-los.
        throw error;
    }
    // Para objetos que não são Erros lançados
    throw new Error('Ocorreu um erro inesperado. Verifique o console para mais detalhes.');
  }
}
...
export async function transcribeAudio(audioBase64: string, mimeType: string): Promise<string> {
    ...
    const textPart = { text: "Transcreva este áudio, que está em alemão. Forneça apenas o texto transcrito, sem qualquer outra formatação ou comentário." };
