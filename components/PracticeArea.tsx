...
import type { Scenario, Message, Feedback, PracticeMode, Persona, OverallFeedback } from '../types';
import { continueConversation, getInlineFeedback, generateAudio, translateText, getSuggestion, getOverallFeedback, ai } from '../geminiService';
        ...
        <div className="flex items-center mt-2 space-x-4">
            ...
            {!isUser && message.text && (
                 <button 
                    ...
                    title={message.translation ? "Ocultar tradução" : "Traduzir"}
                >
                    {message.isTranslating ? 'Traduzindo...' : (message.translation ? 'Ocultar 🇧🇷' : 'Traduzir 🇧🇷')}
                </button>
            )}
        </div>
      </div>
...
interface PracticeAreaProps {
  ...
  mode: PracticeMode;
  // Fix: Update onEndSession signature
  onEndSession: (feedback: OverallFeedback) => void;
  onExit: () => void;
  isReadOnly?: boolean;
  showError: (message: string) => void;
}

const PracticeArea: React.FC<PracticeAreaProps> = ({ scenario, persona, initialMessages, mode, onEndSession, onExit, isReadOnly = false, showError }) => {
  ...
  // Fix: Add logic to handle initial audio generation for conversation mode
  useEffect(() => {
    const firstMessage = messages[0];
    if (mode === 'conversation' && firstMessage?.role === 'model' && firstMessage.isAudioLoading) {
      const fetchAndPlayInitialAudio = async () => {
        try {
            const audioB64 = await generateAudio(firstMessage.text, persona.voice);
            ...
            showError(e instanceof Error ? e.message : 'Falha ao gerar o áudio inicial.');
            setMessages(prev => prev.map(m => m.id === firstMessage.id ? { ...m, isAudioLoading: false } : m));
        }
      };
      fetchAndPlayInitialAudio();
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialMessages, mode, persona.voice, playAudio, showError]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
        ...
        showError(error instanceof Error ? error.message : 'Falha ao processar sua mensagem.');
        setIsLoading(false);
  ...
  const handleGetSuggestions = async () => {
    ...
    try {
        ...
        showError(error instanceof Error ? error.message : 'Falha ao buscar sugestões.');
  ...
  const handleGetFeedback = async (messageId: string, text: string) => {
    try {
        ...
        showError(error instanceof Error ? error.message : 'Falha ao gerar feedback para a frase.');
    }
  };
  ...
  const handleTranslateMessage = async (messageId: string, text: string) => {
    ...
    try {
        ...
        showError(error instanceof Error ? error.message : 'Falha ao traduzir o texto.');
        setMessages(prev => prev.map(m => m.id === messageId ? { ...m, isTranslating: false } : m));
    }
  };
  
  // Fix: New handler for ending the session
  const handleEndSessionClick = async () => {
    setIsLoading(true);
    try {
        const feedback = await getOverallFeedback(messages, scenario);
        onEndSession(feedback);
    } catch (error) {
        showError(error instanceof Error ? error.message : "Falha ao gerar o feedback final.");
    } finally {
        setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const setupAudio = async () => {
      try {
        ...
        showError("O acesso ao microfone é necessário para o modo de conversa. Por favor, habilite nas configurações do seu navegador.");
        onExit();
      }
    };
    ...
    const startLiveSession = async () => {
        ...
        try {
            sessionPromise.current = ai.live.connect({
                ...
                callbacks: {
                    ...
                    onerror: (e) => {
                        ...
                        showError('Ocorreu um erro na sessão de conversa. A conexão pode ter sido perdida.');
                    },
            ...
            showError(error instanceof Error ? error.message : 'Não foi possível iniciar a sessão de conversa.');
        }
    }
    startLiveSession();
  ...
  }, [mode, isReadOnly, scenario.persona, onExit, persona, showError]);
  ...
  const isInputDisabled = isReadOnly || isLoading;
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 font-sans">
      <header className="p-4 bg-white border-b border-gray-200 flex justify-between items-center shadow-sm">
        ...
        <div>
          <button onClick={handleEndSessionClick} disabled={isInputDisabled} className="px-4 py-2 text-sm font-semibold rounded-lg text-white bg-german-red hover:bg-red-700 transition-colors disabled:bg-red-300">
            {isLoading ? 'Aguarde...' : 'Encerrar Conversa'}
          </button>
        </div>
      </header>
